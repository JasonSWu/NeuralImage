...
def train(base_llm, decoder, mem_layer, train_dataloader, num_epochs, PAD_IDX, dim_emb, max_len, device="cuda"):
  ...
  for epoch in range(1, num_epochs+1):
    ...
    for convo in tqdm(train_dataloader):
        ...
        for i in range(n_replies):
          ...
          memories = mem_layer(pooled, torch.concat(memories, dim=0), torch.concat(keys, dim=0))
...
chatbot = FineTuneTransformer(pretrained_model, decoder, hidden_size, 101, 102, device)
